...
{
    [InitializeOnLoad]
    public static class MerchantBoatModelSetup
    {
        private const string Root = "Assets/_Project/Art/Ships/MerchantBoat";
        private const string Model = Root + "/Models/SM_MerchantBoat.fbx";
        private const string MaterialPath = Root + "/Materials/MAT_MerchantBoat_URP.mat";
        private const string PrefabPath = "Assets/_Project/Resources/SeabornMerchantBoatVisual.prefab";
        private static bool building;

        static MerchantBoatModelSetup()
        {
        ...
        private static string TexturePath(string suffix) =>
            Root + "/Textures/T_MerchantBoat_" + suffix + ".png";

        [MenuItem("Seaborn/Art/Build Merchant Boat")]
        public static void Build()
        {
            if (building || EditorApplication.isPlayingOrWillChangePlaymode) return;
            if (!InputsExist())
            {
                Debug.LogWarning("Merchant boat files missing. Extract Seaborn_MerchantBoat_2K.zip into the project root.");
                return;
            }
            building = true;
            ...
            try
            {
                ...
                root = new GameObject("Seaborn Merchant Boat Visual");
                var visual = Object.Instantiate(model, root.transform);
                visual.name = "Merchant Boat Model";
                // Preserve the FBX importer's axis correction (the Meshy model has an authored -90 X rotation).
                // Turn the merchant bow toward ship-local +Z while preserving the upright correction.
                ...
                if (PrefabUtility.SaveAsPrefabAsset(root, PrefabPath) == null)
                    throw new System.InvalidOperationException("Could not save merchant boat prefab.");
                AssetDatabase.SaveAssets();
                Debug.Log("Merchant boat ready: textured 2K URP prefab, length 6.5. Check bow direction and waterline in Play Mode.");
            }
            finally
            {
    ...
    internal sealed class MerchantBoatAssetPostprocessor : AssetPostprocessor
    {
            ...
            string[] moved, string[] movedFrom)
        {
            ...
            {
                if (!path.StartsWith("Assets/_Project/Art/Ships/MerchantBoat/")) continue;
                EditorApplication.delayCall += MerchantBoatModelSetup.TryBuildMissing;
                break;
            }
        }
    }
