...
{
    [InitializeOnLoad]
    public static class RazorwindModelSetup
    {
        private const string Root = "Assets/_Project/Art/Ships/Razorwind";
        private const string Model = Root + "/Models/SM_Razorwind.fbx";
        private const string MaterialPath = Root + "/Materials/MAT_Razorwind_URP.mat";
        private const string PrefabPath = "Assets/_Project/Resources/SeabornRazorwindVisual.prefab";
        private static bool building;

        static RazorwindModelSetup()
        {
        ...
        private static string TexturePath(string suffix) =>
            Root + "/Textures/T_Razorwind_" + suffix + ".png";

        [MenuItem("Seaborn/Art/Build Razorwind")]
        public static void Build()
        {
            if (building || EditorApplication.isPlayingOrWillChangePlaymode) return;
            if (!InputsExist())
            {
                Debug.LogWarning("Razorwind files missing. Extract Seaborn_Razorwind_2K.zip into the project root.");
                return;
            }
            building = true;
            ...
            try
            {
                ...
                root = new GameObject("Seaborn Razorwind Visual");
                var visual = Object.Instantiate(model, root.transform);
                visual.name = "Razorwind Model";
                // Preserve the FBX importer's axis correction (the Meshy model has an authored -90 X rotation).
                // Turn the Razorwind bow toward ship-local +Z while preserving the upright correction.
                ...
                if (PrefabUtility.SaveAsPrefabAsset(root, PrefabPath) == null)
                    throw new System.InvalidOperationException("Could not save Razorwind prefab.");
                AssetDatabase.SaveAssets();
                Debug.Log("Razorwind ready: textured 2K URP prefab, length 6.5. Check bow direction and waterline in Play Mode.");
            }
            finally
            {
        ...
        // Measured from the supplied Azure Corsair mesh, after heading, 6.5 length
        // and 12% waterline correction. Keep these under the visual buoyancy root.
        private static void BuildMuzzles(Transform root)
        {
            Vector3[] port = {
                new Vector3(-0.955f, 0.290f, 0.131f),
                new Vector3(-0.989f, 0.254f, -0.557f),
                new Vector3(-0.949f, 0.257f, -1.314f)
            };
            Vector3[] starboard = {
                new Vector3(0.960f, 0.306f, 0.143f),
                new Vector3(0.991f, 0.256f, -0.557f),
                new Vector3(0.954f, 0.246f, -1.307f)
            };
            for (int i = 0; i < 3; i++)
            {
    ...
    internal sealed class RazorwindAssetPostprocessor : AssetPostprocessor
    {
            ...
            string[] moved, string[] movedFrom)
        {
            ...
            {
                if (!path.StartsWith("Assets/_Project/Art/Ships/Razorwind/")) continue;
                EditorApplication.delayCall += RazorwindModelSetup.TryBuildMissing;
                break;
            }
        }
    }
