...
{
    [InitializeOnLoad]
    public static class StormjawModelSetup
    {
        private const string Root = "Assets/_Project/Art/Creatures/Stormjaw";
        private const string Model = Root + "/Models/SM_Stormjaw.fbx";
        private const string MaterialPath = Root + "/Materials/MAT_Stormjaw_URP.mat";
        private const string PrefabPath = "Assets/_Project/Resources/SeabornStormjawVisual.prefab";
        private static bool building;

        static StormjawModelSetup()
        {
        ...
        private static string TexturePath(string suffix) =>
            Root + "/Textures/T_Stormjaw_" + suffix + ".png";

        [MenuItem("Seaborn/Art/Build Stormjaw")]
        public static void Build()
        {
            if (building || EditorApplication.isPlayingOrWillChangePlaymode) return;
            if (!InputsExist())
            {
                Debug.LogWarning("Stormjaw files missing. Extract Seaborn_Stormjaw_2K.zip into the project root.");
                return;
            }
            building = true;
            ...
            try
            {
                ...
                material.SetFloat("_Cull", 2f); // Closed Stormjaw surface uses back-face culling.
                ...
                root = new GameObject("Seaborn Stormjaw Visual");
                var visual = Object.Instantiate(model, root.transform);
                visual.name = "Stormjaw Model";
                // Preserve the authored FBX upright correction. Stoneback Leviathan already faces +Z.
                ...
                if (PrefabUtility.SaveAsPrefabAsset(root, PrefabPath) == null)
                    throw new System.InvalidOperationException("Could not save Stormjaw prefab.");
                AssetDatabase.SaveAssets();
                Debug.Log("Stormjaw ready: textured 2K URP prefab, length 4.2. Check swimming direction and sea height in Play Mode.");
            }
            finally
            {
    ...
    internal sealed class StormjawAssetPostprocessor : AssetPostprocessor
    {
            ...
            string[] moved, string[] movedFrom)
        {
            ...
            {
                if (!path.StartsWith("Assets/_Project/Art/Creatures/Stormjaw/")) continue;
                EditorApplication.delayCall += StormjawModelSetup.TryBuildMissing;
                break;
            }
        }
    }
